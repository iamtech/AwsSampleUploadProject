 ...
 * (C:\\Users\\amit.singh\\.aws\\credentials) where the sample code will load the credentials from.
 * <p>
...
public class S3TransferProgressSample {
    ...
    public void uploadFile(ProfileCredentialsProvider credentials) throws Exception {
    	
    	ProfileCredentialsProvider credentialsProvider = credentials;
            ...
            ProgressListener progressListener = new ProgressListener() {
                public void progressChanged(ProgressEvent progressEvent) {
                    ...
                    case TRANSFER_COMPLETED_EVENT:
                        pb.setValue(100);
                        AmazonSESSample ses = new AmazonSESSample();
                        try {
							ses.sendEmail(credentialsProvider);
						} catch (IOException e1) {
							throw new AmazonSimpleEmailServiceException(
				                     "Error at sending email" + e1.getMessage());
						}
                        break;
    ...
    private void createAmazonS3Bucket() {
        try {
            if (tx.getAmazonS3Client().doesBucketExistV2(bucketName) == false) {
